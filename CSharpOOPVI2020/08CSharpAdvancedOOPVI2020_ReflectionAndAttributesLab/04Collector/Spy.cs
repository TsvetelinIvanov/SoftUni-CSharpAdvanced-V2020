...
namespace Stealer
{
    public class Spy
    {
        ...
        public string CollectGettersAndSetters(string investgatedClassName)
        {
            Type investigatedClassType = Type.GetType(investgatedClassName);
            MethodInfo[] allMethods = investigatedClassType.GetMethods(BindingFlags.Static | BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            StringBuilder gettersAndSettersBuilder = new StringBuilder();
            foreach (MethodInfo getter in allMethods.Where(m => m.Name.StartsWith("get")))
            {
                gettersAndSettersBuilder.AppendLine(getter.Name + " will return " + getter.ReturnType);
            }

            foreach (MethodInfo setter in allMethods.Where(m => m.Name.StartsWith("set")))
            {
                gettersAndSettersBuilder.AppendLine(setter.Name + " will set field of " + setter.GetParameters().First().ParameterType);
            }

            return gettersAndSettersBuilder.ToString().TrimEnd();
        }
    }
}